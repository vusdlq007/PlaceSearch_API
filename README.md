...
- RDB(MariaDB)에서 일어날 수 있는 많은 커넥션, 잦은 조회로 인한 성능저하 등의 이유로 인해 자주 Read 일경우 readOnly 옵션을 주어 컨텍스트에서 분리, 변경감지 미실행등의 이점을 가져가려했고 장기적으론 자주 조회가 일어나는 단순 조회조건을가진 데이터들은
Redis와 같은 In-memory db로 분산시켜 부하를 나눌수있게 고려하였습니다.
